...
"""
function make_grid(eq::Equation, scenario::Scenario, gridsize_1d, size)
    gridsize = gridsize_1d^2
    dofs = Array{Float64,2}(undef, (get_ndofs(eq), gridsize))
    flux = similar(dofs, 2, get_ndofs(eq), gridsize)
    cellsize = size ./ gridsize_1d
    cells = make_mesh(eq, scenario, gridsize_1d, cellsize, [0.0,0.0])
    Grid(cells, size, dofs, flux, -1.0, 0.0)
end
